...
def _weather_no_indoor_message(db, district: str) -> str:
    """Accurate bad-weather message: only claim a district has no indoor
    places when it genuinely has none; otherwise indoor options exist but
    none matched the generated itinerary."""
    from app.logic.weather.weather_adapter import _count_indoor_places
    if _count_indoor_places(db, district) > 0:
        return (
            f"No suitable indoor places available in {district} for your "
            f"preferences. Try a different category or mobility level."
        )
    return (
        f"No indoor places available in {district}. "
        f"This district is mainly for outdoor activities. Consider travelling "
        f"when weather conditions become favourable."
    )
...
def generate_from_hotel(
    ...
    try:
        ...
        # ── Weather-Aware: router-only overlay ──
        # The base itinerary is always generated by Riddhi's original engine.
        # Only when the user explicitly picks "Generate Weather-Aware" do we
        # swap outdoor places for indoor alternatives on days whose forecast
        # is actually bad. "Continue Anyway" and the normal flow are untouched.
        ...
        if weather_action == "weather_aware":
            ...
            from app.logic.weather.weather_adapter import _count_indoor_places, _find_indoor_places
            ...
            for day in result.get("itinerary", []):
                d = day.get("district", "")
                day_num = day.get("day")
                if not day_bad_weather.get(day_num, False):
                    continue
                ...
                for p in day.get("places", []):
                    ...
                    if io in ("indoor", "both") or is_trek:
                        ...
                        outdoor_slots.append(p)

                if outdoor_slots:
                    candidates = _find_indoor_places(db, d)
                    replacements = []
                    used = set()
                    for slot in outdoor_slots:
                        chosen = None
                        cat = (slot.get("category") or "").lower()
                        for c in candidates:
                            cid = c.get("place_id")
                            if cid in used:
                                continue
                            if cat and cat in (c.get("category") or "").lower():
                                chosen = c
                                break
                        if chosen is None:
                            for c in candidates:
                                if c.get("place_id") not in used:
                                    chosen = c
                                    break
                        if chosen is None:
                            break
                        used.add(chosen.get("place_id"))
                        replacements.append({
                            "type": "place",
                            "place_id": chosen.get("place_id"),
                            "name": chosen.get("place_name"),
                            "category": chosen.get("category"),
                            "duration": chosen.get("duration_hours", 2.0),
                            "is_anchor_activity": False,
                            "indoor_outdoor": (chosen.get("indoor_outdoor") or "indoor").lower(),
                            "weather_sensitive": str(chosen.get("weather_sensitivity", "No")).lower() == "yes",
                            "transport_mode": "Private Car / Local Bus",
                            "travel_dist_km": slot.get("travel_dist_km", 0.0),
                            "latitude": chosen.get("latitude"),
                            "longitude": chosen.get("longitude"),
                            "start_time": slot.get("start_time"),
                            "time_of_day": slot.get("time_of_day"),
                            "district": chosen.get("district") or d,
                            "_weather_replacement": True,
                        })
                    kept = kept + replacements

                day["places"] = kept
                if not kept:
                    msg = _weather_no_indoor_message(db, d)
                    day["weather_message"] = msg
                    removals.append({"district": d, "warning": msg})
